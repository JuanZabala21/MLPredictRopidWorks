...
ProyectosPorMes=[5,8,7,12,7,15,12,16,4,19,10,11 ]
...
for epoca in range(1000):
    fig.clear()
    entrenar_, resPerdida=sess.run([entrenar,perdida],feed_dict={x: Mes,y:ProyectosPorMes})
    # Obtenemos el resultado de la inclinacion (Weight)
    resW=sess.run(W)
    # Obtenemos el resultado del sesgo (Bias)
    resB=sess.run(b)
    # Pintamos las ventas en la grafica
    plt.plot(Mes,ProyectosPorMes,'ro',label="datos")
